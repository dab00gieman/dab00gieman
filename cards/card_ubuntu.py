...
def _prompt(out, x, y, cmd=None, size=14):
    segs = [("dab00gieman", GREEN, True), ("@", GREEN, True),
...
def render(data=None):
    ...
    lines.append(("body", [("dab00gieman", WHITE, True), (" — ML engineer", WHITE, False)]))
    ...
    out = [
        ...
        f'stroke="{BORDER}" stroke-width="1"/>\n',
        # centered title
        svg.text(W / 2, HEADER_H / 2 + 5, "dab00gieman@github: ~", TITLE, 12,
                 "middle", weight="bold"),
    ]
    ...
    for kind, payload in lines:
        if kind == "prompt":
            end_x = _prompt(out, TEXT_X, y, payload)
